...
{if $products}
    {if !$refresh}
        <div class="wishlistLinkTop">
            <p class="wishlisturl form-group">
                <label>{l s='Permalink' mod='rvwishlistblock'}:</label>
                <input type="text" class="form-control" value="{$link->getModuleLink('rvwishlistblock', 'view', ['token' => $token_wish])|escape:'html':'UTF-8'}" readonly="readonly"/>
            </p>
            <div id="showSendWishlist">
                <a class="btn btn-primary send" href="#" onclick="$('#rvsendwishlist-modal').modal('show');" title="{l s='Send this wishlist' mod='rvwishlistblock'}">
                    <span>{l s='Send this wishlist' mod='rvwishlistblock'}</span>
                </a>
            </div>
        </div>
    {/if}
                ...
                <article id="wlp_{$product.id_product}_{$product.id_product_attribute}" class="product-miniature js-product-miniature col-xs-12 col-sm-6 col-md-4 col-lg-6 col-xl-4" data-id-product="{$product.id_product}" data-id-product-attribute="{$product.id_product_attribute}" itemscope itemtype="http://schema.org/Product">
                    <div class="product-container">
                        <div class="thumbnail-container">
                            ...
                            <a class="lnkdel" href="javascript:;" onclick="WishlistProductManage('wlp_bought', 'delete', '{$id_wishlist}', '{$product.id_product}', '{$product.id_product_attribute}', $('#quantity_{$product.id_product}_{$product.id_product_attribute}').val(), $('#priority_{$product.id_product}_{$product.id_product_attribute}').val());" title="{l s='Delete' mod='rvwishlistblock'}">
                        ...
                        <div class="product-description">
                            ...
                            <p class="form-group">
                                <label for="quantity_{$product.id_product}_{$product.id_product_attribute}">
                                    {l s='Quantity' mod='rvwishlistblock'}
                                </label>
                            ...
                            <p class="form-group">
                                <label for="priority_{$product.id_product}_{$product.id_product_attribute}">
                                    {l s='Priority' mod='rvwishlistblock'}:
                                </label>
                                ...
                                {foreach name=wl from=$wishlists item=wishlist}
                                    ...
                                    {if $id_wishlist != {$wishlist.id_wishlist}}
                                                        <option title="{$wishlist.name|escape:'html':'UTF-8'}" value="{$wishlist.id_wishlist}" onclick="wishlistProductChange({$product.id_product}, {$product.id_product_attribute}, '{$id_wishlist}', '{$wishlist.id_wishlist}');">
                                                                {l s='Move to %s'|sprintf:$wishlist.name mod='rvwishlistblock'}
                                                        </option>
                                    {/if}
    ...
    <div id="rvsendwishlist-modal" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
            ...
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h2 class="modal-title">
                        {l s='Send Wishlist' mod='rvwishlistblock'}
                    </h2>
                </div>
                        ...
                        <fieldset>
                            <div class="form-group row">
                                <label for="email1" class="col-md-3 form-control-label required">
                                    {l s='Email' mod='rvwishlistblock'}1
                                </label>
                                <div class="col-md-9">
                            ...
                            {section name=i loop=11 start=2}
                                <div class="form-group row">
                                    <label for="email{$smarty.section.i.index}" class="col-md-3 form-control-label required">
                                        {l s='Email' mod='rvwishlistblock'}{$smarty.section.i.index}
                                    </label>
                                    <div class="col-md-9">
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-dismiss="modal">{l s='Close' mod='rvwishlistblock'}</button>
                    <button id="submitRvWishlist" class="btn btn-primary" type="submit" name="submitWishlist" onclick="WishlistSend('wl_send', '{$id_wishlist}', 'email');">
                        <i class="fa fa-spinner fa-pulse"></i>
                        <span>{l s='Send' mod='rvwishlistblock'}</span>
                    </button>
                </div>
            </div>
        </div>
...
{else}
    <p class="alert alert-warning">
        {l s='No products' mod='rvwishlistblock'}
    </p>
{/if}
