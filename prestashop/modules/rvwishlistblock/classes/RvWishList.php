...
class RvWishList extends ObjectModel
{
    ...
    */
    ...
    {
        Db::getInstance()->execute('DELETE FROM `'._DB_PREFIX_.'rvwishlist_email` WHERE `id_wishlist` = '.(int)($this->id));
        Db::getInstance()->execute('DELETE FROM `'._DB_PREFIX_.'rvwishlist_product` WHERE `id_wishlist` = '.(int)($this->id));
        if ($this->default) {
            $result = Db::getInstance()->executeS('SELECT * FROM `'._DB_PREFIX_.'rvwishlist` WHERE `id_customer` = '.(int)$this->id_customer.' AND `id_wishlist` != '.(int)$this->id.' LIMIT 1');
            foreach ($result as $res) {
                Db::getInstance()->update('rvwishlist', array('default' => '1'), 'id_wishlist = '.(int)$res['id_wishlist']);
            }
        }
    ...
    */
    public static function incCounter($id_wishlist)
    {
        ...
        return Db::getInstance()->execute('
            UPDATE `'._DB_PREFIX_.'rvwishlist` SET
    ...
    */
    public static function getProductByIdCustomer($id_wishlist, $id_customer, $id_lang, $id_product = null, $quantity = false)
    {
        ...
        $products = Db::getInstance()->executeS('
            ...
            FROM `'._DB_PREFIX_.'rvwishlist_product` wp
            ...
            LEFT JOIN `'._DB_PREFIX_.'rvwishlist` w ON w.`id_wishlist` = wp.`id_wishlist`
    ...
    */
    public static function getInfosByIdCustomer($id_customer)
    {
        ...
        return (Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
            ...
            FROM `'._DB_PREFIX_.'rvwishlist_product` wp
            INNER JOIN `'._DB_PREFIX_.'rvwishlist` w ON (w.`id_wishlist` = wp.`id_wishlist`)
            WHERE w.`id_customer` = '.(int)($id_customer).'
            '.$shop_restriction.'
    ...
    */
    public static function addProduct($id_wishlist, $id_customer, $id_product, $id_product_attribute, $quantity)
    {
        ...
        $result = Db::getInstance()->getRow('
            SELECT wp.`quantity`
            FROM `'._DB_PREFIX_.'rvwishlist_product` wp
            JOIN `'._DB_PREFIX_.'rvwishlist` w ON (w.`id_wishlist` = wp.`id_wishlist`)
        ...
        if (empty($result) === false and sizeof($result)) {
            if (($result['quantity'] + $quantity) <= 0) {
                return (RvWishList::removeProduct($id_wishlist, $id_customer, $id_product, $id_product_attribute));
            } else {
                return (Db::getInstance()->execute('
                    UPDATE `'._DB_PREFIX_.'rvwishlist_product` SET
            ...
            return (Db::getInstance()->execute('
                INSERT INTO `'._DB_PREFIX_.'rvwishlist_product` (`id_wishlist`, `id_product`, `id_product_attribute`, `quantity`, `priority`) VALUES(
                '.(int)($id_wishlist).',
                '.(int)($id_product).',
    ...
    */
    public static function removeProduct($id_wishlist, $id_customer, $id_product, $id_product_attribute)
    {
        ...
        $result = Db::getInstance()->getRow('
            ...
            FROM `'._DB_PREFIX_.'rvwishlist` w
            LEFT JOIN `'._DB_PREFIX_.'rvwishlist_product` wp ON (wp.`id_wishlist` = w.`id_wishlist`)
            WHERE `id_customer` = '.(int)($id_customer).'
        ...
        return Db::getInstance()->execute('
            DELETE FROM `'._DB_PREFIX_.'rvwishlist_product`
            WHERE `id_wishlist` = '.(int)($id_wishlist).'
            AND `id_product` = '.(int)($id_product).'
            AND `id_product_attribute` = '.(int)($id_product_attribute));
    }
    ...
    */
    public static function addEmail($id_wishlist, $email)
    {
        ...
        return (Db::getInstance()->execute('
            INSERT INTO `'._DB_PREFIX_.'rvwishlist_email` (`id_wishlist`, `email`, `date_add`) VALUES(
            '.(int)($id_wishlist).',
            \''.pSQL($email).'\',
    ...
    */
    public static function getEmail($id_wishlist, $id_customer)
    {
        ...
        return (Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
            ...
            FROM `'._DB_PREFIX_.'rvwishlist_email` we
            INNER JOIN `'._DB_PREFIX_.'rvwishlist` w ON w.`id_wishlist` = we.`id_wishlist`
    ...
    */
    ...
    {
        return (Bool)Db::getInstance()->getValue('SELECT * FROM `'._DB_PREFIX_.'rvwishlist` WHERE `id_customer` = '.$id_customer.' AND `default` = 1');
    }

    public static function getDefault($id_customer)
    {
        return Db::getInstance()->executeS('SELECT * FROM `'._DB_PREFIX_.'rvwishlist` WHERE `id_customer` = '.$id_customer.' AND `default` = 1');
    }

    /**
    * Set current RvWishList as default
    *
    * @return boolean
    */
    public function setDefault()
    {
        if ($default = $this->getDefault($this->id_customer)) {
            Db::getInstance()->update('rvwishlist', array('default' => '0'), 'id_wishlist = '.$default[0]['id_wishlist']);
        }

        return Db::getInstance()->update('rvwishlist', array('default' => '1'), 'id_wishlist = '.$this->id);
    }
}
