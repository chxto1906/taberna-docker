...
require_once(_PS_MODULE_DIR_.'rvcompare/classes/ProductAttribute.php');

class RvcompareCompareModuleFrontController extends ModuleFrontController
{
    public $php_self;
    ...
    public function displayAjax()
    {
        ...
        if (Tools::getValue('ajax') && Tools::getValue('id_product') && Tools::getValue('action')) {
            if (Tools::getValue('action') == 'add') {
                ...
                if (RvCompareProduct::getNumberProducts($id_compare) < Configuration::get('RVCOMPARATOR_MAXITEM')) {
                    RvCompareProduct::addCompareProduct($id_compare, (int)Tools::getValue('id_product'));
                } else {
                    $this->ajaxDie('0');
                }
            } elseif (Tools::getValue('action') == 'remove') {
                if (isset($this->context->cookie->id_compare)) {
                    RvCompareProduct::removeCompareProduct((int)$this->context->cookie->id_compare, (int)Tools::getValue('id_product'));
                } else {
                    $this->ajaxDie('0');
                }
            } else {
    ...
    public function initContent()
    {
        ...
        RvCompareProduct::cleanCompareProducts('week');

        $hasProduct = false;

        if (!Configuration::get('RVCOMPARATOR_MAXITEM') || !Configuration::get('RVCOMPARATOR_ENABLE')) {
            return Tools::redirect('index.php?controller=404');
        }

        $ids = null;
        if (isset($this->context->cookie->id_compare)) {
            $ids = RvCompareProduct::getCompareProducts($this->context->cookie->id_compare);
        }

        if ($ids) {
            if (count($ids) > 0) {
                if (count($ids) > Configuration::get('RVCOMPARATOR_MAXITEM')) {
                    $ids = array_slice($ids, 0, Configuration::get('RVCOMPARATOR_MAXITEM'));
                }
                ...
                foreach ($ids as $k => &$id) {
                    ...
                    if (!Validate::isLoadedObject($curProduct) || !$curProduct->active || !$curProduct->isAssociatedToShop()) {
                        if (isset($this->context->cookie->id_compare)) {
                            RvCompareProduct::removeCompareProduct($this->context->cookie->id_compare, $id);
                        }
                        unset($ids[$k]);
                        continue;
                    }
                ...
                if (count($listProducts) > 0) {
                    ...
                    $object = new RvCompareProduct((int)$this->context->cookie->id_compare);
        ...
        $this->setTemplate('module:rvcompare/views/templates/hook/products-comparison.tpl');
    }
    ...
    public function getBreadcrumbLinks()
    {
        ...
        $breadcrumb['links'][] = [
            ...
            'url' => $this->context->link->getModuleLink('rvcompare', 'compare'),
        ];
    ...
    {
        $entity = 'module-rvcompare-'.$this->php_self;
