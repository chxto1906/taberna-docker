...
class KbMobileApp extends Module
{
    ...
    private function captureStoreURL($type)
    {
        $ch = curl_init();
        $postdata =  array();
        $store_url = Tools::getHttpHost(false).__PS_BASE_URI__;
        $postdata['store_url'] = $store_url;
        $postdata['platform'] = 'Prestashop';
    ...
    private function alterDatabaseTables()
    {
        ...
        if ($row != "enum('product','category','home','CMS')") {
            $banner_table_query = "ALTER TABLE `" . _DB_PREFIX_ . "kbmobileapp_banners`
                    CHANGE `redirect_activity` `redirect_activity` ENUM('product','category','home','CMS')
    ...
    public function getContent()
    {
        ...
        if (Tools::getIsset('saveBannerSliderFormData') && Tools::getValue('saveBannerSliderFormData')) {
            ...
            $sql= 'insert into ' . _DB_PREFIX_ . 'kbmobileapp_banners
                        ...
                        '.$id_component.','.$id_component_type.',"'.psql($countdown).'",'.$redirect_product_id.','.$category_id.',"'.$redirect_activity.'","'.$image_url.'","'.$image_type.'","'.$product_name.'","'.$image_path.'","'.psql($image_content_mode).'","'.psql($background_color).'","'.psql($timer_text_color).'","'.psql($heading_of_each_banner_data).'","'.$is_enabled_background_color.'")';
            /*end:changes made by knowband on 25 april 2019 to add banner heading*/
            if (Db::getInstance()->execute($sql)) {
                $result =  Db::getInstance(_PS_USE_SQL_SLAVE_)->Insert_ID();
...
//                        id_category="' . pSQL($id_category_array) . '", image_url = "'. psql($image_name_array).'", image_content_mode = "'. psql($image_content_mode).'" where id_component=' . (int) $id_component;
                /*start:changes made by aayushi on 25 march 2019 to add category heading*/
                $sql = 'update ' . _DB_PREFIX_ . 'kbmobileapp_top_category set 
                        id_category="' . pSQL($id_category_array) . '", image_url = "'. psql($image_name_array).'", image_content_mode = "'. psql($image_content_mode).'", category_heading ="' . psql($category_heading_array1). '"where id_component=' . (int) $id_component;
                /*end:changes made by aayushi on 25 march 2019 to add category heading*/
                if (Db::getInstance(_PS_USE_SQL_SLAVE_)->execute($sql)) {
...
//                            '.$id_component.',"'.psql($id_category_array).'","'.psql($image_content_mode).'", "'.psql($image_name_array).'")';
                ...
                $sql= 'insert into ' . _DB_PREFIX_ . 'kbmobileapp_top_category
                            ...
                            '.$id_component.',"'.psql($id_category_array).'","'.psql($image_content_mode).'", "'.psql($image_name_array).'", "'.psql($category_heading_array1).'")';
                /*end:changes made by aayushi on 25 march 2019 to add category heading*/
                if (Db::getInstance()->execute($sql)) {
                    $result =  Db::getInstance(_PS_USE_SQL_SLAVE_)->Insert_ID();
                    $this->getTopcategoryForm(true);
        ...
        if (Tools::getIsset('getTabIconImageUrl') && Tools::getValue('getTabIconImageUrl')) {
            ...
            echo Tools::jsonEncode($module_dir . 'kbmobileapp/'.$tab_icon_data[0]['tab_icon_url']);
            die();
        }
        ...
        if (Tools::getIsset('saveTabIconData') && Tools::getValue('saveTabIconData')) {
            ...
            $sql = 'Select kb_tab_icon_id From ' . _DB_PREFIX_ . 'kb_mobileapp_tab_bar where kb_tab_icon_id= ' . (int)$id;
            $exists = DB::getInstance(_PS_USE_SQL_SLAVE_)->getValue($sql);
            if ($exists) {
...
//                        'desc' => $this->l('Upload your Image')
        ...
        // changes by rishabh jain
        $this->context->controller->addJs($this->_path . 'views/js/admin/layout1.js');
...
//        $bannerslist = $this->getSliderList('banner', false);
    ...
    public function hookDisplayHeader()
    {
        ...
        if ((isset($this->context->cookie->kbmobileapp) && $this->context->cookie->kbmobileapp) ||(isset($this->context->cookie->kbmobileappcontactus) && ($this->context->cookie->kbmobileappcontactus))) {
            if (Configuration::get('KB_MOBILEAPP_CSS') && Configuration::get('KB_MOBILEAPP_CSS') != '') {
                $custom_css = Tools::unserialize(Configuration::get('KB_MOBILEAPP_CSS'));
                $custom_css = urldecode($custom_css);
        ...
        $this->slider_fields_form = array(
                'form' => array(
                    ...
                    'input' => array(
                        ...
                        array(
                            'label' => $this->l('Heading of Category1'),
                            'type' => 'text',
                            'lang' => true,
                            'hint' => $this->l('Enter Heading of Category1'),
                            'class' => '',
                        ...
                        /*start:changes made by aayushi on 25  march 2019 to add category headings*/
                        array(
                            'label' => $this->l('Heading of Category2'),
                            'type' => 'text',
                            'lang' => true,
                            'hint' => $this->l('Enter Heading of Category2'),
                            'class' => '',
                        ...
                        /*start:changes made by aayushi on 25  march 2019 to add category headings*/
                        array(
                            'label' => $this->l('Heading of Category3'),
                            'type' => 'text',
                            'lang' => true,
                            'hint' => $this->l('Enter Heading of Category3'),
                            'class' => '',
                        ...
                        /*start:changes made by aayushi on 25  march 2019 to add category headings*/
                        array(
                            'label' => $this->l('Heading of Category4'),
                            'type' => 'text',
                            'lang' => true,
                            'hint' => $this->l('Enter Heading of Category4'),
                            'class' => '',
                        ...
                        /*start:changes made by aayushi on 25  march 2019 to add category headings*/
                        array(
                            'label' => $this->l('Heading of Category5'),
                            'type' => 'text',
                            'lang' => true,
                            'hint' => $this->l('Enter Heading of Category5'),
                            'class' => '',
                        ...
                        /*start:changes made by aayushi on 25  march 2019 to add category headings*/
                        array(
                            'label' => $this->l('Heading of Category6'),
                            'type' => 'text',
                            'lang' => true,
                            'hint' => $this->l('Enter Heading of Category6'),
                            'class' => '',
                        ...
                        /*start:changes made by aayushi on 25  march 2019 to add category headings*/
                        array(
                            'label' => $this->l('Heading of Category7'),
                            'type' => 'text',
                            'lang' => true,
                            'hint' => $this->l('Enter Heading of Category7'),
                            'class' => '',
                        ...
                        /*start:changes made by aayushi on 25  march 2019 to add category headings*/
                        array(
                            'label' => $this->l('Heading of Category8'),
                            'type' => 'text',
                            'lang' => true,
                            'hint' => $this->l('Enter Heading of Category8'),
                            'class' => '',
...
//                where id_component = '.(int)$id_component;
        /*25  march 2019*/
        $sql = 'SELECT id_category,image_url,image_content_mode,category_heading  FROM ' . _DB_PREFIX_ . 'kbmobileapp_top_category
                where id_component = '.(int)$id_component;
        /*25  march 2019*/
        $categories = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS($sql);
        if (count($categories) > 0) {
...
//        $languages = Language::getLanguages();
    ...
    */
    ...
    public function getTabBarList($ajax)
    {
        ...
        $sql = 'SELECT * FROM ' . _DB_PREFIX_ . 'kb_mobileapp_tab_bar';
        $row = Db::getInstance()->executeS($sql);
        $number_of_entries = count($row);
        $i = 0;
        while ($number_of_entries != 0) {
        ...
        if (count($check_tab_table_result) == 0) {
            $sql= 'insert into ' . _DB_PREFIX_ . 'kb_mobileapp_tab_bar
                    ...
                    `tab_icon_text`
                     ...
                     "1","'.psql($image_name).'","Home","'.psql($tab_bar_text).'")';
            Db::getInstance()->execute($sql);
        }
    }
