...
//        $this->content['newproducts'] = $this->getNewProductsList();
        ...
        $this->content['fontStyle'] = Configuration::get('KBMOBILEAPP_FONT');
        ...
        $this->content['is_tabbar_enabled'] = Configuration::get('tab_bar_status');
        $this->content['tabBar'] = $this->getTabBarData();
        $tab_bar_background_color = explode('#', Configuration::get('tab_bar_background_color'));
        $this->content['tabBar']['tab_bar_background_color'] = $tab_bar_background_color[1];
        $tab_bar_tint_color = explode('#', Configuration::get('tab_bar_tint_color'));
        $this->content['tabBar']['tab_bar_tint_color'] = $tab_bar_tint_color[1];
        $tab_bar_background_color_disabled = explode('#', Configuration::get('tab_bar_background_color_disabled'));
        $this->content['tabBar']['tab_bar_background_color_disabled'] = $tab_bar_background_color_disabled[1];
        $this->content['total_cart_items'] = Cart::getNbProducts($this->context->cart->id);
        $email = Tools::getValue('email', '');
        if (isset($email) && !empty($email)) {
            $this->content['total_wishlist_items'] = $this->getWishListCount($this->context->customer->id);
        }
        ...
        $background_color = Configuration::get('KBMOBILEAPP_APP_BACKGROUND_COLOR');
        if ($button_color == '') {
            $button_color = '#F65200';
        }
        if ($theme_color == '') {
        ...
        $background_color_array = explode('#', $background_color);
        $this->content['app_background_color']= $background_color_array[1];
     ...
     * Get Tab Bar data
     *
     * @return array tab bar data
     */
    public function getTabBarData()
    {
        $data = array();
        $tab_icon_text_array = array();
        ...
        foreach ($tab_icon_data as $tab_key => $tabs) {
            $tab_icon_text_array = Tools::unSerialize($tabs['tab_icon_text']);
            $id = Language::getIdByIso(Tools::getValue('iso_code'), false);
            if (isset($tab_icon_text_array[$id])) {
                if (empty($tab_icon_text_array[$id])) {
                    $data['tabs'][$tab_key]['title'] = $tab_icon_text_array[Configuration::get('PS_LANG_DEFAULT')];
                } else {
                    $data['tabs'][$tab_key]['title'] = $tab_icon_text_array[$id];
                }
            } else {
                $data['tabs'][$tab_key]['title'] = $tab_icon_text_array[Configuration::get('PS_LANG_DEFAULT')];
            }
            $data['tabs'][$tab_key]['image_src'] = $module_dir . 'kbmobileapp/'.$tabs['tab_icon_url'];
            $data['tabs'][$tab_key]['redirect_to'] = $tabs['tab_icon_redirect_activity'];
        }
        return $data;
    }
    /*end: changes made by Aayushi Agarwal on 15 April 2019  to add tab bar functionality*/
...
//        }
                ...
                case 'CAT':
                    ...
                    if (!empty($results)) {
                        foreach ($results as $m) {
                            /*Start:changes added by aayushi on 25 april 2019 to add category icons*/
                            $data =  new Category($m['id_category']);
                            ...
                            $data = (array)$data;
                            $id_image = $m['id_category'];
                            if (isset($id_image) && !empty($id_image)) {
                                $image_url =  $link->getPageLink('index').$id_image.'.jpg';
                                $image_url = $http_string.$link->getCatImageLink($m['name'], $m['id_category'])."?time_stamp=" . time();
                            } else {
                                $image_url = '';
                            }
                            /*end:changes added by aayushi on 25 april 2019 to add category icons*/
                            if ($m['id_category'] == $id) {
    ...
    public function getSubChild($id_parent, $categories, $further_child = true)
    {
        ...
        /*start:changes added by knowband to add category icons*/
        if ($this->checkSecureUrl()) {
            $module_dir = _PS_BASE_URL_SSL_ . __PS_BASE_URI__ . str_replace(_PS_ROOT_DIR_ . '/', '', _PS_IMG_DIR_);
            $http_string = 'https://';
        } else {
        ...
        /*end:changes added by knowband to add category icons*/
        foreach ($categories as $cat) {
            /*Start:changes added by knowband to add category icons*/
            $data =  new Category($cat['id_category']);
            $id_image = $data->id_image;
            $link = new link();
            if (isset($id_image) && !empty($id_image)) {
                $image_url =  $link->getPageLink('index').$id_image.'.jpg';
                $image_url = $http_string.$link->getCatImageLink($cat['name'], $cat['id_category'])."?time_stamp=" . time();
            } else {
                $image_url = '';
            }
            /*end:changes added by knowband to add category icons*/
