...
$(document).ready(function()
{
    ...
    $('#tab_bar_enable_form_form').addClass('col-lg-10 col-md-9');
    $("#tab_bar_enable_form_form").css("float", "right");
    ...
    /*end*/
    /*start: changes made by Aayushi Agarwal on 15 April 2019  to add tab bar functionality*/
    else if ( default_tab == 'tabbarsettings') {
        change_tab($("#link-"+default_tab), 8);
    }
    /*end: changes made by Aayushi Agarwal on 15 April 2019  to add tab bar functionality*/
...
//            }
    ...
    /*start:changes made by aayushi agarwal on 15 April 2019 to make tab bar*/
     $('.kb_tab_bar_setting_btn').click(function() {
        return veloValidateTabBarEnableForm(this);
    });

    /*end:changes made by aayushi agarwal on 15 April 2019 to make tab bar*/
...
//                $('#vss-button').appendTo($(".panel-heading"));
        ...
        $('#vss-validate-button').hide();
        /*start: changes made by Aayushi Agarwal on 15 April 2019  to add tab bar functionality*/
        $('#form-kb_tab_bar_list').hide();
        $('#vss-add-new-tab-icon-button').hide();
        $('#tab_bar_enable_form_form').hide();
        /*end: changes made by Aayushi Agarwal on 15 April 2019  to add tab bar functionality*/
        closePaymentMethod();
        hideSliderForm();
    } else if (b == 3) {
...
//        hideSliderForm();
    ...
    /*start:chnages done by aayushi on 15 april 2019 to add tab bar functionality*/
    else if (b == 6) {
        $("[id^='fieldset'] h3 span").html(sliders_settings);
        $(".panel-heading span").html(sliders_settings);
        $('#general_form').hide();
        ...
        /*start: changes made by Aayushi Agarwal on 1 Dec 2018 for shipping methods*/
        //$('#google_setup_form').show();
        $('#form-kb_shipping_list').hide();
        /*end*/
        $('#facebook_setup_form').hide();
        $('#vss-validate-button').hide();
         /*start: changes made by Aayushi Agarwal on 15 April 2019  to add tab bar functionality*/
        //$('#vss-add-new-tab-icon-button').show();
        $('#form-kb_tab_bar_list').show();
        $('#vss-add-new-tab-icon-button').appendTo($("#form-kb_tab_bar_list .panel-heading"));
        $('#vss-add-new-tab-icon-button').show();
        $('#tab_bar_enable_form_form').show();
        /*end: changes made by Aayushi Agarwal on 15 April 2019  to add tab bar functionality*/
        closePaymentMethod();
        hideSliderForm();
    //}    
...
/*start:changes made by aayushi agarwal on 15 April 2019 to make tab bar*/
function veloValidateTabBarEnableForm(button_ele) {
    $('.kb_tab_bar_setting_btn').attr('disabled', 'disabled');
    $('input[name="tab_bar_status"]').parent().removeClass('kb_error_field');
    var is_tab_error = false;
    var tab_bar_error = false;
    $('.kb_error_message').remove();
    if ($('input[name="tab_bar_status"]:checked').val() == "1") {
        $.ajax({
        ...
        success: function (data)
        {
            if (data < 2) {
                is_tab_error = true;
                //alert(is_tab_error);
                tab_bar_error = tab_bar_error_msg;
                $('input[name="tab_bar_status"]').parent().addClass('kb_error_field');
                $('input[name="tab_bar_status"]').parent().after('<span class="kb_error_message">' + tab_bar_error + '</span>');
            } else {
                $('.kb_tab_bar_setting_btn').removeAttr('disabled');
            }
        }
    });
    }
...
function veloValidateBannerSliderForm(event)
{
    ...
    $('input[name="filename"]').removeClass('kb_error_field');
    /*start:changes made by knowband on 8 march 2019 to add CMS page redirect activity*/
    var redirect_cms_id = $.trim($('#redirect_banner_cms_name').val());
    /*END:changes made by knowband on 8 march 2019 to add CMS page redirect activity*/
