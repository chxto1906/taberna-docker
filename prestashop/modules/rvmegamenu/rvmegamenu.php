...
include_once (_PS_MODULE_DIR_ . 'rvmegamenu/classes/RvMegamenuClass.php');
include_once (_PS_MODULE_DIR_ . 'rvmegamenu/classes/RvMegamenuRowClass.php');
include_once (_PS_MODULE_DIR_ . 'rvmegamenu/classes/RvMegamenuColumnClass.php');
include_once (_PS_MODULE_DIR_ . 'rvmegamenu/classes/RvMegamenuItemClass.php');
include_once (_PS_MODULE_DIR_ . 'rvmegamenu/sql/SampleDataMenu.php');

class Rvmegamenu extends Module implements WidgetInterface
{
    ...
    public function __construct()
    {
        ...
        $this->bootstrap = true;
        parent::__construct();
        $this->displayName = $this->getTranslator()
            ->trans('RV - Megamenu', array() , 'Modules.RvMegamenu.Admin');
        $this->description = $this->getTranslator()
            ->trans('Show Megamenus on your site', array() , 'Modules.RvMegamenu.Admin');
    ...
    {
        if (parent::install() && $this->registerHook('header') && $this->registerHook('displayMegamenu') && $this->registerHook('displayMegamenuMobile') && $this->registerHook('actionShopDataDuplication') && $this->registerHook('actionObjectLanguageAddAfter'))
        {
    ...
    public function getContent()
    {
        ...
        elseif ((Tools::isSubmit('buildMenu') || Tools::isSubmit('submitRow') || Tools::isSubmit('delete_row')) && Tools::isSubmit('id_rvmegamenu') || Tools::isSubmit('changeStatusRow') || Tools::isSubmit('submitCol') || Tools::isSubmit('delete_col') || Tools::isSubmit('submitSubItem') || Tools::isSubmit('delete_submenu_item') || Tools::isSubmit('changeStatusSubItem'))
        {
            $this->_postProcess();
            $this->html .= $this->renderBuildMenu();
        }
        elseif (Tools::isSubmit('addMenuItem') || Tools::isSubmit('id_item')) $this->html .= $this->renderAddMeniItem();
        elseif (Tools::isSubmit('addCol') || Tools::isSubmit('id_column')) $this->html .= $this->renderAddCol();
        elseif (Tools::isSubmit('addRow') || Tools::isSubmit('id_row')) $this->html .= $this->renderAddRow();
        elseif (Tools::isSubmit('addMenu') || (Tools::isSubmit('id_rvmegamenu') && $this->menuExists(Tools::getValue('id_rvmegamenu')))) $this->html .= $this->renderAddForm();
        else
        {
            $this->_postProcess();
            $this
    ...
    public function renderList()
    {
        $this
            ->context
            ->controller
            ->addJqueryUI('ui.sortable');
        $info_menus = $this->getMenuInfo();
        foreach ($info_menus as $key => $info_menu)
        {
            $info_menus[$key]['status'] = $this->displayStatus($info_menu['id_rvmegamenu'], $info_menu['active']);
    ...
    public function getMenuInfo($active = null)
    {
        ...
        return Db::getInstance(_PS_USE_SQL_SLAVE_)
            ->executeS('
			SELECT pc.*, pl.*
			FROM ' . _DB_PREFIX_ . 'rvmegamenu_shop pc
			LEFT JOIN ' . _DB_PREFIX_ . 'rvmegamenu_lang pl ON (pc.id_rvmegamenu = pl.id_rvmegamenu AND pc.`id_shop` = pl.`id_shop`)
			WHERE pl.id_shop = ' . $id_shop . ' AND pl.id_lang = ' . $id_lang . ($active ? ' AND pc.`active` = 1' : ' ') . ' ORDER BY pc.position ASC, pc.id_rvmegamenu ASC');
    }

    public function displayStatus($id_rvmegamenu, $active)
    {
    ...
    protected function _postProcess()
    {
        ...
        {
            $this->_clearCache('rvmegamenu.tpl');
            if (Tools::getValue('id_rvmegamenu'))
            {
                $menu_item = new RvMegamenuClass((int)Tools::getValue('id_rvmegamenu'));
                $pos_arr = $menu_item->getPositionByMenu((int)Tools::getValue('id_rvmegamenu'));
                $menu_item->position = $pos_arr['position'];
                if (!Validate::isLoadedObject($menu_item))
                {
                    $this->html .= $this->displayError($this->l('Invalid id_rvmegamenu'));
                    return false;
                }
            }
            else
            {
                $menu_item = new RvMegamenuClass();
            ...
            { // icon img
                ...
                if (isset($image_up) && $image_up != '') $menu_item->icon = $image_up;
            }
            ...
            if (!$errors)
            {
                if (!Tools::getValue('id_rvmegamenu'))
                {
                    if (!$menu_item->add())
                    {
                        ...
                        Tools::redirectAdmin($this
                            ->context
                            ->link
                            ->getAdminLink('AdminModules', true) . '&conf=1&configure=' . $this->name . '&tab_module=' . $this->tab . '&module_name=' . $this->name);
                    }
                }
                else
                {
        ...
        elseif (Tools::isSubmit('changeStatus') && Tools::isSubmit('id_rvmegamenu'))
        {
            $this->_clearCache('rvmegamenu.tpl');
            $menu = new RvMegamenuClass((int)Tools::getValue('id_rvmegamenu'));
            if ($menu->active == 0) $menu->active = 1;
            else $menu->active = 0;

            $pos_arr = $menu->getPositionByMenu((int)Tools::getValue('id_rvmegamenu'));
            ...
            Tools::redirectAdmin($this
                ->context
                ->link
                ->getAdminLink('AdminModules', true) . '&conf=6&configure=' . $this->name . '&tab_module=' . $this->tab . '&module_name=' . $this->name);
        }
        elseif (Tools::isSubmit('removeIcon') && Tools::isSubmit('id_rvmegamenu'))
        {
            $this->_clearCache('rvmegamenu.tpl');
            $menu = new RvMegamenuClass((int)Tools::getValue('id_rvmegamenu'));
            $icon = $menu->icon;
            if (preg_match('/sample/', $icon) === 0) if ($icon && file_exists(_PS_MODULE_DIR_ . 'rvmegamenu/views/img/icons/' . $icon)) @unlink(_PS_MODULE_DIR_ . 'rvmegamenu/views/img/icons/' . $icon);
            $menu->icon = '';
            $menu->update();
        }
        elseif (Tools::isSubmit('submitRow'))
        {
            ...
            {
                $row_item = new RvMegamenuRowClass(Tools::getValue('id_row'));
            ...
            else $row_item = new RvMegamenuRowClass();
            $row_item->active = (int)Tools::getValue('active');
            $row_item->id_rvmegamenu = (int)Tools::getValue('id_rvmegamenu');
            ...
            if (!$errors)
            {
                if (!Tools::getValue('id_row'))
                {
                    if (!$row_item->add()) $errors[] = $this->displayError($this->l('The row_item could not be added.'));
                }
                else
                {
                    if (!$row_item->update()) $errors[] = $this->displayError($this->l('The row_item could not be updated.'));
                }
            }
            return $errors;
        }
        ...
        {
            $this->_clearCache('rvmegamenu.tpl');
            $menu_item = new RvMegamenuClass((int)Tools::getValue('delete_id_menu'));
            $res = $menu_item->delete();
            if (!$res) $this->html .= $this->displayError('Could not delete.');
            else Tools::redirectAdmin($this
                ->context
                ->link
                ->getAdminLink('AdminModules', true) . '&conf=1&configure=' . $this->name . '&tab_module=' . $this->tab . '&module_name=' . $this->name . '&id_rvmegamenu=' . Tools::getValue('id_rvmegamenu'));
        }
        elseif (Tools::isSubmit('delete_row'))
        {
            $this->_clearCache('rvmegamenu.tpl');
            $row_item = new RvMegamenuRowClass((int)Tools::getValue('id_row'));
            $res = $row_item->delete();
            if (!$res) $this->html .= $this->displayError('Could not delete.');
            else Tools::redirectAdmin($this
                ->context
                ->link
                ->getAdminLink('AdminModules', true) . '&conf=1&configure=' . $this->name . '&tab_module=' . $this->tab . '&module_name=' . $this->name . '&id_rvmegamenu=' . Tools::getValue('id_rvmegamenu') . '&buildMenu=1');
        }
        elseif (Tools::isSubmit('changeStatusRow') && Tools::isSubmit('id_row'))
        {
            ...
            $id_shop = (int)$this
                ->context
                ->shop->id;
            $row = new RvMegamenuRowClass((int)Tools::getValue('id_row') , null, $id_shop);
        ...
        elseif (Tools::isSubmit('submitCol'))
        {
            ...
            {
                $col_item = new RvMegamenuColumnClass(Tools::getValue('id_column'));
            ...
            {
                $col_item = new RvMegamenuColumnClass();
            ...
            if (!$errors)
            {
                if (!Tools::getValue('id_column'))
                {
                    if (!$col_item->add()) $errors[] = $this->displayError($this->l('The col_item could not be added.'));
                }
                else
                {
                    if (!$col_item->update()) $errors[] = $this->displayError($this->l('The col_item could not be updated.'));
                }
            }
            return $errors;
        }
        ...
        {
            $this->_clearCache('rvmegamenu.tpl');
            $col_item = new RvMegamenuColumnClass((int)Tools::getValue('id_column'));
            $res = $col_item->delete();
            if (!$res) $this->html .= $this->displayError('Could not delete.');
            else Tools::redirectAdmin($this
                ->context
        ...
        elseif (Tools::isSubmit('submitSubItem'))
        {
            ...
            {
                $sub_menu_item = new RvMegamenuItemClass(Tools::getValue('id_item'));
            ...
            {
                $sub_menu_item = new RvMegamenuItemClass();
            ...
            if (!$errors)
            {
                if (!Tools::getValue('id_item'))
                {
                    if (!$sub_menu_item->add()) $errors[] = $this->displayError($this->l('The sub_menu_item could not be added.'));
                }
                else
                {
                    if (!$sub_menu_item->update()) $errors[] = $this->displayError($this->l('The sub_menu_item could not be updated.'));
                }
            }
            return $errors;
        }
        ...
        {
            $this->_clearCache('rvmegamenu.tpl');
            $submenu_item = new RvMegamenuItemClass((int)Tools::getValue('id_item'));
        ...
        elseif (Tools::isSubmit('changeStatusSubItem') && Tools::isSubmit('id_item'))
        {
            $this->_clearCache('rvmegamenu.tpl');
            $subMenu_item = new RvMegamenuItemClass((int)Tools::getValue('id_item'));
            if ($subMenu_item->active == 0) $subMenu_item->active = 1;
            else $subMenu_item->active = 0;
    ...
    public function renderAddForm()
    {
        $this
            ->context
            ->controller
            ->addJS($this->_path . 'views/js/back.js');
        $this
            ->context
            ->controller
            ->addCSS($this->_path . 'views/css/back.css');
        $id_rvmegamenu = Tools::getValue('id_rvmegamenu');
        if (isset($id_rvmegamenu)) $menu = new RvMegamenuClass($id_rvmegamenu);
        else $menu = new RvMegamenuClass();
    ...
    public function renderAddMeniItem()
    {
        ...
        $fields_form = array(
            'form' => array(
                ...
                'buttons' => array(
                    array(
                        'href' => AdminController::$currentIndex . '&configure=' . $this->name . '&token=' . Tools::getAdminTokenLite('AdminModules') . '&id_rvmegamenu=' . $id_rvmegamenu . '&buildMenu=1',
                        'title' => $this->l('Cancel') ,
        ...
        if (Tools::isSubmit('id_item'))
        {
            ...
            $subMenu_item = new RvMegamenuItemClass(Tools::getValue('id_item'));
        ...
        $helper->currentIndex = $this
            ->context
            ->link
            ->getAdminLink('AdminModules', false) . '&configure=' . $this->name . '&tab_module=' . $this->tab . '&module_name=' . $this->name . '&id_rvmegamenu=' . $id_rvmegamenu . '&buildMenu=1';
    ...
    public function getAddFieldsValuesLink()
    {
        ...
        if (Tools::isSubmit('id_item'))
        {
            $menu_item = new RvMegamenuItemClass((int)Tools::getValue('id_item'));
        ...
        {
            $menu_item = new RvMegamenuItemClass();
    ...
    public function renderAddRow()
    {
        ...
        if (Tools::isSubmit('id_row')) $fields_form['form']['input'][] = array(
            'type' => 'hidden',
            'name' => 'id_row'
        );
        $fields_form['form']['input'][] = array(
    ...
    public function getAddFieldsValues()
    {
        ...
        {
            $row = new RvMegamenuRowClass((int)Tools::getValue('id_row'));
            $fields['id_row'] = (int)Tools::getValue('id_row', $row->id);
            $fields['active'] = Tools::getValue('active', $row->active);
            $fields['id_rvmegamenu'] = Tools::getValue('id_rvmegamenu', $row->id_rvmegamenu);
            $fields['class'] = Tools::getValue('class', $row->class);
        }
        else
        {
            $row = new RvMegamenuRowClass();
            $fields['active'] = Tools::getValue('active', 1);
            $fields['id_rvmegamenu'] = Tools::getValue('id_rvmegamenu', 1);
    ...
    public function renderAddCol()
    {
        ...
        if (Tools::isSubmit('id_column')) $fields_form['form']['input'][] = array(
            'type' => 'hidden',
            'name' => 'id_column'
        );
        $fields_form['form']['input'][] = array(
    ...
    public function getAddFieldsValuesCol()
    {
        ...
        {
            $col = new RvMegamenuColumnClass((int)Tools::getValue('id_column'));
    ...
    public function renderBuildMenu()
    {
        ...
        $this
            ...
            ->controller
            ->addCSS($this->_path . 'views/css/back.css');
        $id_rvmegamenu = Tools::getValue('id_rvmegamenu', 1);
        $info_rows = $this->getRowInfo($id_rvmegamenu);

        foreach ($info_rows as $key => $info_row)
        {
            $info_rows[$key]['status'] = $this->displayStatusRow($id_rvmegamenu, $info_row['id_row'], $info_row['active']);
    ...
    public function getRowInfo($id_menu, $active = null)
    {
        ...
        $row_infos_rs = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			SELECT mr.*
			FROM ' . _DB_PREFIX_ . 'rvmegamenu_row_shop mr
			WHERE mr.`id_shop` = ' . $id_shop . ' AND mr.id_rvmegamenu = ' . $id_menu . ($active ? ' AND mr.`active` = 1' : ' '));
        if (is_array($row_infos_rs) && count($row_infos_rs) > 0) $row_infos = $row_infos_rs;
        return $row_infos;
    }
    ...
    public function getColInfo($id_row, $active = null, $is_backend = true)
    {
        ...
        $cols_result = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			SELECT mc.*
			FROM ' . _DB_PREFIX_ . 'rvmegamenu_column_shop mc
			WHERE mc.`id_shop` = ' . $id_shop . ' AND mc.id_row = ' . $id_row . ($active ? ' AND mc.`active` = 1' : '') . ' ORDER BY mc.position ASC, mc.id_column ASC');

        if (is_array($cols_result) && count($cols_result) > 0) $col_infos = $cols_result;

        if (is_array($col_infos) && count($col_infos) > 0) foreach ($col_infos as $key => $col_info)
        {
            $sub_menu_infos = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
					SELECT smi.*,smil.*
					FROM ' . _DB_PREFIX_ . 'rvmegamenu_item_shop smi
					LEFT JOIN ' . _DB_PREFIX_ . 'rvmegamenu_item_lang smil ON (smi.id_item = smil.id_item AND smi.id_shop = smil.id_shop)
            ...
            if (is_array($sub_menu_infos) && count($sub_menu_infos) > 0)
            {
                ...
                {
                    $id_rvmegamenu = Tools::getValue('id_rvmegamenu', 1);
                    if (isset($id_rvmegamenu) && $id_rvmegamenu > 0 && $is_backend) $sub_menu_infos[$key1]['status'] = $this->displayStatusSubItem($id_rvmegamenu, $sub_menu_info['id_item'], $sub_menu_info['active']);
                    ...
                    if ($sub_menu_info['type_link'] == 4)
                    {
                        ...
                        if (isset($id_prod) && $id_prod > 0)
                        {
                            ...
                            foreach ($productsImages as $pi) $productarr[$pi['id_product']] = $pi;
            ...
            else $col_infos[$key]['list_menu_item'] = array();
        }
        return $col_infos;
    }
    ...
    public function getProductById($id_prod)
    {
        ...
        $sql = 'SELECT p.*, product_shop.*, stock.out_of_stock, IFNULL(stock.quantity, 0) as quantity,
			...
			' . (Combination::isFeatureActive() ? ', product_attribute_shop.minimal_quantity AS product_attribute_minimal_quantity, IFNULL(product_attribute_shop.`id_product_attribute`,0) id_product_attribute' : '') . '
				...
				ON (p.`id_product` = product_attribute_shop.`id_product` AND product_attribute_shop.`default_on` = 1 AND product_attribute_shop.id_shop=' . (int)$context
            ->shop->id . ')' : '') . '
				' . Product::sqlStock('p', 0) . '
				LEFT JOIN `' . _DB_PREFIX_ . 'image_shop` image_shop
					ON (image_shop.`id_product` = p.`id_product` AND image_shop.cover=1 AND image_shop.id_shop=' . (int)$context
            ->shop->id . ')
    ...
    public function getSubMenu($id_rvmegamenu)
    {
        $info_rows = array();
        if (is_array($this->getRowInfo($id_rvmegamenu, true)) && count($this->getRowInfo($id_rvmegamenu, true)) > 0)
        {
            $info_rows = $this->getRowInfo($id_rvmegamenu, true);
            foreach ($info_rows as $key => $info_row) $info_rows[$key]['list_col'] = $this->getColInfo($info_row['id_row'], true, false);
        }
        return $info_rows;
    }
    ...
    {
        $req = 'SELECT rv.`id_rvmegamenu` as id_rvmegamenu
				FROM `' . _DB_PREFIX_ . 'rvmegamenu` rv
				WHERE rv.`id_rvmegamenu` = ' . (int)$id;
    ...
    public function getWidgetVariables($hookName = null, array $configuration = [])
    {
        $id_lang = (int)$this
            ->context
            ->language->id;
        $id_shop = (int)Context::getContext()
            ->shop->id;
        $group_cat_result = array();
       
            $menu_obj = new RvMegamenuClass();
            ...
            foreach ($menus as $menu)
            {
                ...
                else
                {
                    ...
                    $sub_menu = $this->getSubMenu($menu['id_rvmegamenu']);
                    if (is_array($sub_menu) && count($sub_menu) > 0) $menu['sub_menu'] = $sub_menu;
                    else $menu['sub_menu'] = array();
                }
                $new_menus[] = $menu;
            }

            return ['menus' => $new_menus, 'icon_path' => $this->_path . 'views/img/icons/', '_path' => $this->_path];
    ...
    {
        if (is_null($id_lang)) $id_lang = (int)$this
            ->context
            ->language->id;
        ...
        {
            if ($link) $html .= '<option value="' . $this
                ->context
                ->link
                ->getManufacturerLink($manufacturer['id_manufacturer']) . '">' . $manufacturer['name'] . '</option>';
            else $html .= '<option value="MAN' . (int)$manufacturer['id_manufacturer'] . '">' . $manufacturer['name'] . '</option>';
        }
        $html .= '</optgroup>';
        ...
        foreach ($suppliers as $supplier)
        {
            if ($link) $html .= '<option value="' . $this
                ->context
                ->link
                ->getSupplierLink($supplier['id_supplier']) . '">' . $supplier['name'] . '</option>';
            else $html .= '<option value="SUP' . (int)$supplier['id_supplier'] . '">' . $supplier['name'] . '</option>';
        }
        $html .= '</optgroup>';
        ...
        if (count($shoplink) > 1)
        {
            ...
            foreach ($shoplink as $sh) $html .= '<option value="SHO' . (int)$sh['id_shop'] . '">' . $sh['name'] . '</option>';
        }
        $html .= '</optgroup>';
        return $html;
    }
    ...
    {
        $html = '';
        $id_lang = $id_lang ? (int)$id_lang : (int)Context::getContext()
            ->language->id;
        $id_shop = $id_shop ? (int)$id_shop : (int)Context::getContext()
            ->shop->id;
        ...
        )))
        {
            if ($link) $html .= '<option value="' . $this
                ->context
                ->link
                ->getCategoryLink($category->id) . '">' . (isset($spacer) ? $spacer : '') . str_repeat('&nbsp;', 3 * (int)$category->level_depth) . $category->name . '</option>';
            else $html .= '<option value="CAT' . (int)$category->id . '">' . str_repeat('&nbsp;', 3 * (int)$category->level_depth) . $category->name . '</option>';
        }
        elseif ($category->id != Configuration::get('PS_ROOT_CATEGORY')) $html .= '<optgroup label="' . str_repeat('&nbsp;', 3 * (int)$category->level_depth) . $category->name . '">';
        if (isset($children) && count($children)) foreach ($children as $child)
        {
    ...
    public function getCMSOptions($parent = 0, $depth = 0, $id_lang = false, $link = false)
    {
        ...
        foreach ($categories as $category) $html .= $this->getCMSOptions($category['id_cms_category'], (int)$depth + 1, (int)$id_lang, $link);
        foreach ($pages as $page) if ($link) $html .= '<option value="' . $this
            ->context
            ->link
            ->getCMSLink($page['id_cms']) . '">' . (isset($spacer) ? $spacer : '') . $page['meta_title'] . '</option>';
    ...
    {
        $id_shop = ($id_shop !== false) ? (int)$id_shop : (int)Context::getContext()
            ->shop->id;
        $id_lang = $id_lang ? (int)$id_lang : (int)Context::getContext()
            ->language->id;
    ...
    public function getPagesOption($id_lang = null, $link = false)
    {
        ...
        foreach ($files as $file)
        {
            if ($link) $html .= '<option value="' . $this
                ->context
                ->link
                ->getPageLink($file['page']) . '">' . (($file['title'] != '') ? $file['title'] : $file['page']) . '</option>';
    ...
    public function fomartLink($item = null, $id_lang = null)
    {
        ...
        switch ($type)
        {
            case 'CAT':
                ...
                $link = $this
                    ->context
                    ->link
                    ->getCategoryLink((int)$key, null, $id_lang);
            ...
            case 'CMS':
                ...
                $link = $this
                    ->context
                    ->link
                    ->getCMSLink((int)$key, null, $id_lang, $id_shop);
            ...
            case 'MAN':
                ...
                $link = $this
                    ->context
                    ->link
                    ->getManufacturerLink($man->id, $man->link_rewrite, $id_lang);
                $title = $man->name;
                break;
            case 'SUP':
                ...
                $link = $this
                    ->context
                    ->link
                    ->getSupplierLink($sup->id, $sup->link_rewrite, $id_lang);
                $title = $sup->name;
                break;
            case 'PAG':
                ...
                $link = $this
                    ->context
                    ->link
                    ->getPageLink($pag['page'], true, $id_lang);
                if ($page_name == $pag['page']) $selected_item = true;
                if ($pag['page'] == 'index') $title = $this->l('Home');
        ...
        public function getRespCategories($id_category = 1, $id_lang = false, $id_shop = false, $level_root = 1, $sub_title = '', $type_icon = 1, $icon = '', $sub_align = '', $spec_class = '')
        {
            ...
            if (isset($children) && count($children) && $category->level_depth > 1) $class .= 'parent ';
            if ($spec_class != '') $class .= $spec_class;

            if ($category->level_depth > 1) $cat_link = $category->getLink();
            else $cat_link = $this
                ->context
                ->link
                ->getPageLink('index');
            ...
            if (!empty($is_intersected))
            {
                ...
                $this->respMenu .= '<a class="rvinnermenu" href="' . $cat_link . '">';
                if ($type_icon == 1 && $icon != '') $this->respMenu .= '<i class="material-icons">' . $icon . '</i>';
                elseif ($type_icon == 0 && $icon != '') $this->respMenu .= '<img src="' . $icon_path . $icon . '" alt=""/>';
                $this->respMenu .= '<span class="catagory">' . $category->name . '</span>';
                if ($sub_title != '') $this->respMenu .= '<div class="menu-subtitle">' . $sub_title . '</div>';
                $this->respMenu .= '</a>';
            }
            ...
            if (isset($children) && count($children))
            {
                ...
                foreach ($children as $child) $this->getRespCategories((int)$child['id_category'], (int)$id_lang, (int)$child['id_shop'], $level_root);
                $this->respMenu .= '</ul>';
            }
            if (!empty($is_intersected)) $this->respMenu .= '</li>';
        ...
        public function hookActionShopDataDuplication($params)
        {
			...
			INSERT IGNORE INTO ' . _DB_PREFIX_ . 'rvmegamenu_shop (`id_rvmegamenu`, `id_shop`, `type_link`, `dropdown`, `type_icon`, `icon`, `align_sub`, `width_sub`, `class`, `active`)
			SELECT id_rvmegamenu, ' . (int)$params['new_id_shop'] . ', type_link, dropdown, type_icon, icon, align_sub, width_sub, class, active
			FROM ' . _DB_PREFIX_ . 'rvmegamenu_shop
			WHERE id_shop = ' . (int)$params['old_id_shop']);

            Db::getInstance()->execute('
			INSERT IGNORE INTO ' . _DB_PREFIX_ . 'rvmegamenu_lang (`id_rvmegamenu`, `id_shop`, `id_lang`, `title`, `link`, `subtitle`)
			SELECT id_rvmegamenu, ' . (int)$params['new_id_shop'] . ', id_lang, title, link, subtitle
			FROM ' . _DB_PREFIX_ . 'rvmegamenu_lang
			WHERE id_shop = ' . (int)$params['old_id_shop']);

            Db::getInstance()->execute('
			INSERT IGNORE INTO ' . _DB_PREFIX_ . 'rvmegamenu_row_shop (`id_row`, `id_rvmegamenu`, `id_shop`, `class`, `active`)
			SELECT id_row, id_rvmegamenu, ' . (int)$params['new_id_shop'] . ', class, active
			FROM ' . _DB_PREFIX_ . 'rvmegamenu_row_shop
			WHERE id_shop = ' . (int)$params['old_id_shop']);

            Db::getInstance()->execute('
			INSERT IGNORE INTO ' . _DB_PREFIX_ . 'rvmegamenu_column_shop (`id_column`, `id_row`, `id_shop`, `width`, `class`, `active`)
			SELECT id_column, id_row, ' . (int)$params['new_id_shop'] . ', width, class, active
			FROM ' . _DB_PREFIX_ . 'rvmegamenu_column_shop
			WHERE id_shop = ' . (int)$params['old_id_shop']);

            Db::getInstance()->execute('
			INSERT IGNORE INTO ' . _DB_PREFIX_ . 'rvmegamenu_item_shop (`id_item`, `id_column`, `id_shop`, `type_link`, `type_item`, `id_product`, `active`)
			SELECT id_item, id_column, ' . (int)$params['new_id_shop'] . ', type_link, type_item, id_product, active
			FROM ' . _DB_PREFIX_ . 'rvmegamenu_item_shop
			WHERE id_shop = ' . (int)$params['old_id_shop']);

            Db::getInstance()->execute('
			INSERT IGNORE INTO ' . _DB_PREFIX_ . 'rvmegamenu_item_lang (`id_item`, `id_shop`, `id_lang`, `title`, `link`, `text`)
			SELECT id_item, ' . (int)$params['new_id_shop'] . ', id_lang, title, link, text
			FROM ' . _DB_PREFIX_ . 'rvmegamenu_item_lang
			WHERE id_shop = ' . (int)$params['old_id_shop']);
        }
        public function hookActionObjectLanguageAddAfter($params)
        {
            Db::getInstance()->execute('
			INSERT IGNORE INTO ' . _DB_PREFIX_ . 'rvmegamenu_lang (`id_rvmegamenu`, `id_shop`, `id_lang`, `title`, `link`, `subtitle`)
			SELECT id_rvmegamenu, id_shop, ' . (int)$params['object']->id . ', title, link, subtitle
			FROM ' . _DB_PREFIX_ . 'rvmegamenu_lang
