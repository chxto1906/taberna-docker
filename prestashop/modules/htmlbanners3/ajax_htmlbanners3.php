...
include_once('htmlbanners3.php');

$hero_slider = new HtmlBanners3();
$slides = array();
...
if (Tools::getValue('action') == 'updateSlidesPosition' && Tools::getValue('slides')) {
    ...
    foreach ($slides as $position => $id_slide) {
        $res = Db::getInstance()->execute(
            '
			UPDATE `'._DB_PREFIX_.'htmlbanners3_slides` SET `position` = '.(int)$position.'
			WHERE `id_htmlbanners3_slides` = '.(int)$id_slide
        );
    }
