						...
						<div class="dummyfile input-group">
							...
							<span class="input-group-btn">
								<button id="{$input.name}_{$language.id_lang}-selectbutton" type="button" name="submitAddAttachments" class="btn btn-default">
									<i class="icon-folder-open"></i> {l s='Choose a file' mod='rvtestimonials'}
								</button>
							</span>
						</div>
					</div>
...
{block name="input"}
    ...
    {if $input.name == "RVTESTIMONIALS_IMG"}
        ...
        {if isset($fields_value[$input.name]) && $fields_value[$input.name] != ''}
            ...
            <a href="{$current}&{$identifier}={$form_id}&token={$token}&deleteConfig={$input.name}">
                <i class="icon icon-trash"></i> {l s='Delete' mod='rvtestimonials'}
            </a>
            <small>{l s='Do not forget to save the options after delete the image!' mod='rvtestimonials'}</small>
            <br /><br />
        {/if}
