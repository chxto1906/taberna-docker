...
class sincronizacionwebservicesValidacionProductosBeforeFacturaSAPModuleFrontController extends ModuleFrontController {
    ...
    public function validate($cart) {
        ...
        foreach ($products as $product) {
            ...
            $articulo = $facturaSAP->recuperaArticulo($cod_almacen,$product["reference"]);
            if (is_array($articulo)){
                if ($articulo[0] == "0"){
                    $flag = $facturaSAP->getFlag($articulo[9],$articulo[10]);
                    if (($flag == 3) || ($flag == 2)) {
                        $valido = $facturaSAP->validarLoteSerie($cod_almacen,$product["reference"],$flag,$articulo[19],$articulo[18]);
                        if ((int)$valido != 0 ){
                            $lote = false;
                            $message .= "- Producto: ".$product["name"].", reference: ".$product["reference"]." por el momento no se pudo validar existencia de lote";
                        }
                    }
                    ...
                    if ((int)$stock < (int)$product["cart_quantity"]) {
                        $disponible = false;
                        $message .= "- Producto: ".$product["name"].", reference: ".$product["reference"]." no tiene suficiente stock (".$stock<0?0:$stock.").";
                    }
                }else{
                    $existeArticulo = false;
                    $message .= "- Producto: ".$product["name"].", reference: ".$product["reference"]." por el momento no se encuentra disponible en esta tienda.";
                }
            }else{
                $existeArticulo = false;
                $message .= "- Producto: ".$product["name"].", reference: ".$product["reference"]." por el momento no se encuentra disponible en esta tienda.";
