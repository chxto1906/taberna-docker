<template>
    <div class="verte-picker" ref="picker" :class="`verte-picker--${mode}`">
        <div class="verte-picker__origin" ref="origin">
            <canvas class="verte-picker__canvas" width="250" height="120" ref="canvas" @mousedown="handleSelect" @touchstart="handleSelect"></canvas>
            <div class="verte-picker__cursor" ref="cursor" :style="`transform: translate3d(${cursor.x}px, ${cursor.y}px, 0)`"></div>
        </div>
        <slider class="verte-picker__slider" v-if="mode === 'square'" :gradient="['#f00', '#ff0', '#0f0', '#0ff', '#00f', '#f0f', '#f00']" :editable="false" :max="360" v-model="currentHue"></slider>
        <slider class="verte-picker__slider" v-if="mode === 'wheel'" :gradient="[`hsl(${currentColor.hue},0%,${currentColor.lum}%)`, `hsl(${currentColor.hue},100%,${currentColor.lum}%)`]" :editable="false" :max="100" v-model="currentSat"></slider>
    </div>
</template>
