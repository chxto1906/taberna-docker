...
<div class="slider" ref="wrapper">
    <div class="slider__track" ref="track" v-on="trackSlide ? { mousedown: select, touchstart: select } : { }">
        ...
        <div class="slider__handle" v-for="handle in handles" @mousedown="select" @touchstart="select" :style="`transform: translate(${handle.position}px, 0); background-color: ${handle.color};`">
            <div class="slider__label" v-if="label">{{ handle.value }}</div>
        </div>
    </div><input class="slider__input" ref="input" :type="colorCode ? 'text' : 'number'" v-show="editable" @change="updateValue($event.target.value)" /></div>
</template>
