...
class HtmlBanner5 extends ObjectModel
{
    public $title;
    public $description;
    public $url;
    ...
    public static $definition = array(
        'table' => 'htmlbanners5_slides',
        'primary' => 'id_htmlbanners5_slides',
        'multilang' => true,
        'fields' => array(
    ...
    public function add($autodate = true, $null_values = false)
    {
        ...
        $res &= Db::getInstance()->execute(
            '
            INSERT INTO `'._DB_PREFIX_.'htmlbanners5` (`id_shop`, `id_htmlbanners5_slides`)
    ...
    public function delete()
    {
        ...
        $res &= Db::getInstance()->execute(
            '
            DELETE FROM `'._DB_PREFIX_.'htmlbanners5`
            WHERE `id_htmlbanners5_slides` = '.(int)$this->id
        );
    ...
    public function reOrderPositions()
    {
        ...
        $max = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS(
            ...
            WHERE hss.`id_htmlbanners5_slides` = hs.`id_htmlbanners5_slides` AND hs.`id_shop` = '.(int)$id_shop
        );
        ...
        $rows = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS(
            '
            SELECT hss.`position` as position, hss.`id_htmlbanners5_slides` as id_slide
            FROM `'._DB_PREFIX_.'htmlbanners5_slides` hss
            LEFT JOIN `'._DB_PREFIX_.'htmlbanners5` hs ON (hss.`id_htmlbanners5_slides` = hs.`id_htmlbanners5_slides`)
            WHERE hs.`id_shop` = '.(int)$id_shop.' AND hss.`position` > '.(int)$this->position
        );

        foreach ($rows as $row) {
            $current_slide = new HtmlBanner5($row['id_slide']);
