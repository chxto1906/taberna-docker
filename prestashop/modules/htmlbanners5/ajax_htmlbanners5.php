...
include_once('htmlbanners5.php');

$hero_slider = new HtmlBanners5();
$slides = array();
...
if (Tools::getValue('action') == 'updateSlidesPosition' && Tools::getValue('slides')) {
    ...
    foreach ($slides as $position => $id_slide) {
        $res = Db::getInstance()->execute(
            '
			UPDATE `'._DB_PREFIX_.'htmlbanners5_slides` SET `position` = '.(int)$position.'
			WHERE `id_htmlbanners5_slides` = '.(int)$id_slide
        );
    }
