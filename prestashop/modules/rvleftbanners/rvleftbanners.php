...
include_once(_PS_MODULE_DIR_.'rvleftbanners/RvLeftBanner.php');

class RvLeftBanners extends Module implements WidgetInterface
{
    protected $_html = '';
    ...
    public function __construct()
    {
        ...
        $this->displayName = $this->l('RV - Left Banner Block');
        $this->description = $this->l('Adds an Left Banner to your site.');
        $this->ps_versions_compliancy = array('min' => '1.7.0.0', 'max' => _PS_VERSION_);

        $this->templateFile = 'module:rvleftbanners/views/templates/hook/rvleftbanners.tpl';
    }
    ...
    */
    protected function installSamples()
    {
        ...
        for ($i = 1; $i <= 3; ++$i) {
            $slide = new RvLeftBanner();
            $slide->position = $i;
            $slide->active = 1;
            foreach ($languages as $language) {
                $slide->title[$language['id_lang']] = 'Left Banner '.$i;
                $slide->url[$language['id_lang']] = '#';
                $slide->image[$language['id_lang']] = 'left-banner-'.$i.'.png';
            }
            $slide->add();
        }
    }
    ...
    */
    protected function createTables()
    {
        ...
        $res = (bool)Db::getInstance()->execute('
            CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'rvleftbanners` (
            `id_rvleftbanners_slides` int(10) unsigned NOT NULL AUTO_INCREMENT,
            `id_shop` int(10) unsigned NOT NULL,
            PRIMARY KEY (`id_rvleftbanners_slides`, `id_shop`)
            ) ENGINE='._MYSQL_ENGINE_.' DEFAULT CHARSET=UTF8;
            ');

        /* Slides configuration */
        $res &= Db::getInstance()->execute('
            CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'rvleftbanners_slides` (
            `id_rvleftbanners_slides` int(10) unsigned NOT NULL AUTO_INCREMENT,
            `position` int(10) unsigned NOT NULL DEFAULT \'0\',
            `active` tinyint(1) unsigned NOT NULL DEFAULT \'0\',
            PRIMARY KEY (`id_rvleftbanners_slides`)
            ) ENGINE='._MYSQL_ENGINE_.' DEFAULT CHARSET=UTF8;
            ');

        /* Slides lang configuration */
        $res &= Db::getInstance()->execute('
            CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'rvleftbanners_slides_lang` (
            `id_rvleftbanners_slides` int(10) unsigned NOT NULL,
            `id_lang` int(10) unsigned NOT NULL,
            `title` varchar(255) NOT NULL,
            `url` varchar(255) NOT NULL,
            `image` varchar(255) NOT NULL,
            PRIMARY KEY (`id_rvleftbanners_slides`,`id_lang`)
            ) ENGINE='._MYSQL_ENGINE_.' DEFAULT CHARSET=UTF8;
            ');
    ...
    */
    protected function deleteTables()
    {
        ...
        foreach ($slides as $slide) {
            $to_del = new RvLeftBanner($slide['id_slide']);
            $to_del->delete();
        }

        return Db::getInstance()->execute('
            DROP TABLE IF EXISTS `'._DB_PREFIX_.'rvleftbanners`, `'._DB_PREFIX_.'rvleftbanners_slides`, `'._DB_PREFIX_.'rvleftbanners_slides_lang`;
            ');
    }
    ...
    public function getContent()
    {
        ...
        if (Tools::isSubmit('submitSlide') || Tools::isSubmit('delete_id_slide') ||
            ...
            if ($mode == 'add') {
                ...
                $associated_shop_ids = RvLeftBanner::getAssociatedIdsShop((int)Tools::getValue('id_slide'));
    ...
    protected function _postProcess()
    {
        ...
        if (Tools::isSubmit('changeStatus') && Tools::isSubmit('id_slide')) {
            $slide = new RvLeftBanner((int)Tools::getValue('id_slide'));
            if ($slide->active == 0) {
                $slide->active = 1;
            } else {
                $slide->active = 0;
            ...
            $slide = new RvLeftBanner((int)Tools::getValue('delete_id_slide'));
            $res = $slide->delete();
            $this->clearCache();
            if (!$res) {
    ...
    {
        $this->context->controller->registerStylesheet('modules-rvleftbanners', 'modules/'.$this->name.'/css/rvleftbanners.css', array('media' => 'all', 'priority' => 150));
    }
    ...
    public function hookActionShopDataDuplication($params)
    {
        Db::getInstance()->execute('
            INSERT IGNORE INTO '._DB_PREFIX_.'rvleftbanners (id_rvleftbanners_slides, id_shop)
            SELECT id_rvleftbanners_slides, '.(int)$params['new_id_shop'].'
            FROM '._DB_PREFIX_.'rvleftbanners
    ...
    public function getNextPosition()
    {
        $row = Db::getInstance(_PS_USE_SQL_SLAVE_)->getRow('
            ...
            FROM `'._DB_PREFIX_.'rvleftbanners_slides` hss, `'._DB_PREFIX_.'rvleftbanners` hs
            WHERE hss.`id_rvleftbanners_slides` = hs.`id_rvleftbanners_slides` AND hs.`id_shop` = '.(int)$this->context->shop->id
            );
    ...
    public function getSlides($active = null)
    {
        ...
        $slides = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
            SELECT hs.`id_rvleftbanners_slides` as id_slide, hss.`position`, hss.`active`, hssl.`title`,
            hssl.`url`, hssl.`image`
            FROM '._DB_PREFIX_.'rvleftbanners hs
            LEFT JOIN '._DB_PREFIX_.'rvleftbanners_slides hss ON (hs.id_rvleftbanners_slides = hss.id_rvleftbanners_slides)
            LEFT JOIN '._DB_PREFIX_.'rvleftbanners_slides_lang hssl ON (hss.id_rvleftbanners_slides = hssl.id_rvleftbanners_slides)
        ...
        foreach ($slides as &$slide) {
            $slide['image_url'] = $this->context->link->getMediaLink(_MODULE_DIR_.'rvleftbanners/images/'.$slide['image']);
        }
    ...
    public function getAllImagesBySlidesId($id_slides, $active = null, $id_shop = null)
    {
        ...
        $results = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
            ...
            LEFT JOIN '._DB_PREFIX_.'rvleftbanners_slides hss ON (hs.id_rvleftbanners_slides = hss.id_rvleftbanners_slides)
            LEFT JOIN '._DB_PREFIX_.'rvleftbanners_slides_lang hssl ON (hss.id_rvleftbanners_slides = hssl.id_rvleftbanners_slides)
            WHERE hs.`id_rvleftbanners_slides` = '.(int)$id_slides.' AND hs.`id_shop` = '.(int)$id_shop.
            ($active ? ' AND hss.`active` = 1' : ' ')
            );
    ...
    {
        $req = 'SELECT hs.`id_rvleftbanners_slides` as id_slide
            FROM `'._DB_PREFIX_.'rvleftbanners` hs
            WHERE hs.`id_rvleftbanners_slides` = '.(int)$id_slide;
    ...
    public function renderList()
    {
        ...
        foreach ($slides as $key => $slide) {
            ...
            $associated_shop_ids = RvLeftBanner::getAssociatedIdsShop((int)$slide['id_slide']);
