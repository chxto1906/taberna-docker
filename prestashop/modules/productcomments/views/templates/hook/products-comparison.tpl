    ...
    {foreach from=$list_ids_product item=id_product}
        <td class="{$classname} comparison_infos product-{$id_product}" align="center">
            {if isset($product_comments[$id_product]) AND $product_comments[$id_product]}
                <a href="javascript:void(0);" rel="#comments_{$id_product}" data-id-product-comment="{$id_product|intval}" class="btn btn-primary">{l s='view comments' mod='productcomments'}</a>
                <div style="display:none" id="comments_{$id_product}" class="cluetipblock"> 
                    {foreach from=$product_comments[$id_product] item=comment}
                        <div class="commentcluetip">
