...
/* Include Models */
include_once(dirname(__FILE__).'/../../classes/ProductComment.php');
include_once(dirname(__FILE__).'/../../classes/ProductCommentCriterion.php');
...
class ProductCommentsDefaultModuleFrontController extends ModuleFrontController
{
    ...
    protected function ajaxProcessAddComment()
    {
        ...
        if (!$this->context->customer->id && !Configuration::get('PRODUCTCOMMENT_ALLOW_GUESTS')) {
        ...
        if (!count($errors)) {
            ...
            if (!$customer_comment || ($customer_comment && (strtotime($customer_comment['date_add']) + (int)Configuration::get('PRODUCTCOMMENT_MINIMAL_TIME')) < time())) {
                ...
                $result = true;
                Tools::clearCache(Context::getContext()->smarty, $this->getTemplatePath('productcomments_reviews.tpl'));
            } else {
                $result = false;
                $errors[] = $module_instance->l('Please wait before posting another comment', 'default').' '.Configuration::get('PRODUCTCOMMENT_MINIMAL_TIME').' '.$module_instance->l('seconds before posting a new comment', 'default');
            }
        } else {
            $result = false;
        }
