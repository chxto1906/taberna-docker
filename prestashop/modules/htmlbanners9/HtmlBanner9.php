...
class HtmlBanner9 extends ObjectModel
{
    public $title;
    public $description;
    public $url;
    ...
    public static $definition = array(
        'table' => 'htmlbanners9_slides',
        'primary' => 'id_htmlbanners9_slides',
        'multilang' => true,
        'fields' => array(
    ...
    public function add($autodate = true, $null_values = false)
    {
        ...
        $res &= Db::getInstance()->execute(
            '
            INSERT INTO `'._DB_PREFIX_.'htmlbanners9` (`id_shop`, `id_htmlbanners9_slides`)
    ...
    public function delete()
    {
        ...
        $res &= Db::getInstance()->execute(
            '
            DELETE FROM `'._DB_PREFIX_.'htmlbanners9`
            WHERE `id_htmlbanners9_slides` = '.(int)$this->id
        );
    ...
    public function reOrderPositions()
    {
        ...
        $max = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS(
            ...
            WHERE hss.`id_htmlbanners9_slides` = hs.`id_htmlbanners9_slides` AND hs.`id_shop` = '.(int)$id_shop
        );
        ...
        $rows = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS(
            '
            SELECT hss.`position` as position, hss.`id_htmlbanners9_slides` as id_slide
            FROM `'._DB_PREFIX_.'htmlbanners9_slides` hss
            LEFT JOIN `'._DB_PREFIX_.'htmlbanners9` hs ON (hss.`id_htmlbanners9_slides` = hs.`id_htmlbanners9_slides`)
            WHERE hs.`id_shop` = '.(int)$id_shop.' AND hss.`position` > '.(int)$this->position
        );

        foreach ($rows as $row) {
            $current_slide = new HtmlBanner9($row['id_slide']);
