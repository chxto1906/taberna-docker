...
include_once('htmlbanners9.php');

$hero_slider = new HtmlBanners9();
$slides = array();
...
if (Tools::getValue('action') == 'updateSlidesPosition' && Tools::getValue('slides')) {
    ...
    foreach ($slides as $position => $id_slide) {
        $res = Db::getInstance()->execute(
            '
			UPDATE `'._DB_PREFIX_.'htmlbanners9_slides` SET `position` = '.(int)$position.'
			WHERE `id_htmlbanners9_slides` = '.(int)$id_slide
        );
    }
