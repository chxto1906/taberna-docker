...
include_once('htmlbanners2.php');

$hero_slider = new HtmlBanners2();
$slides = array();
...
if (Tools::getValue('action') == 'updateSlidesPosition' && Tools::getValue('slides')) {
    ...
    foreach ($slides as $position => $id_slide) {
        $res = Db::getInstance()->execute(
            '
			UPDATE `'._DB_PREFIX_.'htmlbanners2_slides` SET `position` = '.(int)$position.'
			WHERE `id_htmlbanners2_slides` = '.(int)$id_slide
        );
    }
