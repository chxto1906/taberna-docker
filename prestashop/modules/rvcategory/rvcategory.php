...
class RvCategory extends Module implements WidgetInterface
{
    ...
    private $rv_category_config;
    public function __construct()
    {
        $this->name = 'rvcategory';
        ...
        $this->bootstrap = true;
        parent::__construct();
        $this->displayName = $this->getTranslator()->trans('RV - Category', array(), 'Modules.RVcategory.Admin');
        $this->description = $this->getTranslator()->trans('Show category with subcategory and category image.', array(), 'Modules.RVcategory.Admin');
        $this->ps_versions_compliancy = array('min' => '1.7.0.0', 'max' => _PS_VERSION_);
        $this->rv_category_config = 'RV_CONFIG_CATEGORY_FEATURE';
    ...
    {
        $this->_clearCache('rvcategory.tpl');
        if (parent::uninstall() == false || !Configuration::deleteByName($this->rv_category_config)) {
            return false;
        }
        return true;
    }
    ...
    public function postProcess()
    {
        if (Tools::isSubmit('saveConfig')) {
            ...
            if (isset($errors) && count($errors)) {
                ...
                if ($config) {
                    Configuration::updateValue($this->rv_category_config, $config);
                    $this->_clearCache('rvcategory.tpl');
    ...
    public function getWidgetVariables($hookName, array $configuration)
    {
        ...
        $categories = $this->callGetCategoryList($id_lang);
            return array(
                'rv_categories' => $categories,
                'rv_config' => $this->config,
                'iso_code' => $iso_code,
