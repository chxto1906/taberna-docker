            ...
            if ($error = ImageManager::validateUpload($FILES['image'], 4000000)) {
                ...
                if (!move_uploaded_file($FILES['image']['tmp_name'], $path)) {
                    ...
                    foreach ($posts_types as $image_type) {
                        $dir = _PS_MODULE_DIR_ .'smartblog/images/'.$id.'-'.Tools::stripslashes($image_type['type_name']).'.jpg';
                        if (file_exists($dir)) {
                            unlink($dir);
                        }
                    }
                    foreach ($posts_types as $image_type) {
                        ImageManager::resize($path, _PS_MODULE_DIR_ .'smartblog/images/'.$id.'-'.Tools::stripslashes($image_type['type_name']).'.jpg', (int)$image_type['width'], (int)$image_type['height']);
                    }
                }
            }
        }
