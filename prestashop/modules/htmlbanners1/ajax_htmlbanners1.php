...
include_once('htmlbanners1.php');

$hero_slider = new HtmlBanners1();
$slides = array();
...
if (Tools::getValue('action') == 'updateSlidesPosition' && Tools::getValue('slides')) {
    ...
    foreach ($slides as $position => $id_slide) {
        $res = Db::getInstance()->execute(
            '
			UPDATE `'._DB_PREFIX_.'htmlbanners1_slides` SET `position` = '.(int)$position.'
			WHERE `id_htmlbanners1_slides` = '.(int)$id_slide
        );
    }
