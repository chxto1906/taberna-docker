...
class HtmlBanner8 extends ObjectModel
{
    public $title;
    public $description;
    public $url;
    ...
    public static $definition = array(
        'table' => 'htmlbanners8_slides',
        'primary' => 'id_htmlbanners8_slides',
        'multilang' => true,
        'fields' => array(
    ...
    public function add($autodate = true, $null_values = false)
    {
        ...
        $res &= Db::getInstance()->execute(
            '
            INSERT INTO `'._DB_PREFIX_.'htmlbanners8` (`id_shop`, `id_htmlbanners8_slides`)
    ...
    public function delete()
    {
        ...
        $res &= Db::getInstance()->execute(
            '
            DELETE FROM `'._DB_PREFIX_.'htmlbanners8`
            WHERE `id_htmlbanners8_slides` = '.(int)$this->id
        );
    ...
    public function reOrderPositions()
    {
        ...
        $max = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS(
            ...
            WHERE hss.`id_htmlbanners8_slides` = hs.`id_htmlbanners8_slides` AND hs.`id_shop` = '.(int)$id_shop
        );
        ...
        $rows = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS(
            '
            SELECT hss.`position` as position, hss.`id_htmlbanners8_slides` as id_slide
            FROM `'._DB_PREFIX_.'htmlbanners8_slides` hss
            LEFT JOIN `'._DB_PREFIX_.'htmlbanners8` hs ON (hss.`id_htmlbanners8_slides` = hs.`id_htmlbanners8_slides`)
            WHERE hs.`id_shop` = '.(int)$id_shop.' AND hss.`position` > '.(int)$this->position
        );

        foreach ($rows as $row) {
            $current_slide = new HtmlBanner8($row['id_slide']);
