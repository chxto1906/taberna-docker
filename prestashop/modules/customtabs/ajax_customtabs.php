...
include_once('customtabs.php');

$hero_slider = new CustomTabs();
$slides = array();
...
if (Tools::getValue('action') == 'updateSlidesPosition' && Tools::getValue('slides')) {
    ...
    foreach ($slides as $position => $id_slide) {
        $res = Db::getInstance()->execute(
            '
			UPDATE `'._DB_PREFIX_.'customtabs_slides` SET `position` = '.(int)$position.'
			WHERE `id_customtabs_slides` = '.(int)$id_slide
        );
    }
