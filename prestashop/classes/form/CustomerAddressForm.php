...
include_once(dirname(__FILE__)."/ValidateCedulaEcuador.php");
//include_once("ValidateCedulaEcuador.php");
...
class CustomerAddressFormCore extends AbstractForm
{
    ...
    public function validate()
    {
        ...
        $dni = $this->getValue('dni');
        $type_dni = $this->getValue('type_dni');
            ...
            if ($type_dni == "Ruc"){
                $is_valid = $this->isRuc($dni);
                if (!$is_valid)
                    $this->getField('dni')->addError($this->translator->trans('RUC no válido.',
                            array(),
                            'Shop.Forms.Errors'));
            } elseif ($type_dni == "Cédula") {
                //$is_valid = $this->isCedula($dni);
                if (!$this->isCedula($dni)){
                    $this->getField('dni')->addError($this->translator->trans('Cédula no válida.',
                            array("key","value"),
                            'Shop.Forms.Errors'));
                    $is_valid = false;
                }
    ...
    public function getTemplateVariables()
    {
        ...
        if (empty($formFields['email']['value'])) {
            $formFields['email']['value'] = $context->customer->email;
        }
