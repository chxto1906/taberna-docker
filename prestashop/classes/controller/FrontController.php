...
class FrontControllerCore extends Controller
{
    ...
    public function checkSession() {
        ...
        if ($object_session){
            ...
            $cart_id = $object_session->cart_id;
            $customer_id = $object_session->customer_id;
    ...
    public function isOpen(){
        ...
        if (count($horarioHoy) == 4) {
            if ((($horaActual > $horarioHoy[0]) && ($horaActual < $horarioHoy[1])) || (($horaActual > $horarioHoy[2]) && ($horaActual < $horarioHoy[3])))
                $open = true;
        } else if (count($horarioHoy) == 2) {
            if (($horaActual > $horarioHoy[0]) && ($horaActual < $horarioHoy[1])) 
                $open = true;    
        }
    ...
    /**** HENRY ******/
    protected function getHoursStoreCurrent($id_shop)
    {
        ...
        return  Db::getInstance()->executeS('
         ...
         AND sl.`id_lang` = 1 AND stsh.`id_shop` = '.$id_shop);
    }
